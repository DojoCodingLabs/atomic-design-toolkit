...
**Added:** discovered during the dojo-os audit (April 2026) — `bun.lock` + `package-lock.json` coexisted under `"packageManager": "bun@1.3.10"`.

---

## Atomic-Design Enforcement Signals (V7)

Bundle health (signals 1-10) measures *what is broken today*. The seven enforcement signals below measure *whether the project will drift again tomorrow*. Codified from the DOJ-3946 component-layer Conway's Law premortem (dojo-os, 2026-05-14) and the four-cure defense-in-depth pattern documented in `references/atomic-design-hooks-setup.md`.
...
**Remediation:** Add a CI step that runs the structural guard script (analog of dojo-os's `primitives-guard.ts` for DOJ-3698). Fail the build on violations; require an approved exception comment to override.
