...
How to install **write-time enforcement hooks** for atomic-design structure in a consumer repository. This is **Cure 4** of the four-cure defense-in-depth pattern documented in the DOJ-3946 component-layer premortem (dojo-os, 2026-05-14).
...
The premortem that motivates this reference cites a forensic finding from the dojo-os engagement:

> "DojoOS lleva 7.3 meses sin shipping. ~37% de effort wasted (~$394K)." — DOJ-2482, quoted verbatim in the DOJ-3946 spike body, 2026-05-08.
...
|--------|--------------|--------|
| Jan-May 2026 (already spent) | $394K | DOJ-2482 / DOJ-3946 |
...
```json
{
  "$schema": "https://make-no-mistakes-toolkit.dojocoding.io/schemas/atomic-design-rules.v1.json",
  "version": 1,
  "stack": "vite",
  "framework": "react",
...
This is what catches the cross-repo Conway's Law multiplication described in the premortem §7.4.2: if dojo-os installs the cure but the next product in the same organization (dojo-agent-openclaw-plugin, future repos) does not, the structural anti-pattern reproduces in the new repo. Toolkit-level hooks scale the discipline to every consumer.
...
- DOJ-3946 premortem (dojo-os) — the full forensic narrative, $394K / $324K / $718K math, 4-cure pattern.
- Existing precedent in dojo-os: `.claude/hooks/pre-write-decision-record-location.sh` (DOJ-3849) — the OpenSpec-convention enforcement hook this pattern is modeled on.
